...
#------------------------------
table(train$country)
table(train$X1_diffSumExports.kmt.)

write.csv(out_mapping, "./Data/mapped_year_linearized.csv",  row.names = FALSE)

colnames(train)

train %>% filter(X1_diffSumExports.kmt. == -16196.6495 ) %>% select(country, ID, month, X1_diffSumExports.kmt., X2_diffSumExports.kmt.)

train %>% filter(country == 52 & (month == 1 | month == 2) & (X1_diffSumExports.kmt. < 0 | X2_diffSumExports.kmt. < 0)) %>% select(ID, month, X1_diffSumExports.kmt., X2_diffSumExports.kmt.)
out_mapping %>% filter(country == 52 & month == 1)